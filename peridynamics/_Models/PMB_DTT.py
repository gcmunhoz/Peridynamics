...
class PMB_DTT(Base):
    ...
    def __init__(self,par_omega,E,G0,bc) -> None:
        ...
        def T(mesh,u,ii,family,bc):
            ...
            µ=damageFactor(self.history.S[ii],ii,family,bc)
            ...
            #New formulation
                ff=(S>S1[0])*(S<S0[0])*(S0[0])*((S-S1[0])/(S0[0] - S1[0]))+(S>=S0[0])*(S<=S0[1])*S+(S>S0[1])*(S<S1[1])*((S0[1])*(S1[1]-S)/((S1[1])-S0[1]))
        ...
        def damageFactor(x,ii,family,bc):
            ...
            if bc.damage.damage_on==True:
                ...
                S0=[-0.98,0.95*Sc] # S0- and S0+
                S1=[-0.99,1.05*Sc] # S1- and S1+
                µ=(x<= S0[1])*(x>=-1)*1 + (x > S0[1])*(x<S1[1])*(S1[1] - x)/(S1[1] - S0[1]);
        ...
        def strainEnergyDensity(mesh,u,family,ii,bc,par_omega) -> float:
            ...
            #noFail=np.logical_or(bc.damage.noFail[ii],bc.damage.noFail[jj])
        ...
        def antiderivative(x,damage,ii):
            ...
            if bc.damage.damage_on==True:
                # Damage dependent crack
                alpha=bc.damage.alpha
                beta=bc.damage.beta
                gamma=bc.damage.gamma
                if bc.damage.phi[ii]>alpha:

                    Sc=bc.damage.Sc*min([gamma,1+beta*(bc.damage.phi[ii]-alpha)/(1-bc.damage.phi[ii])])
                else:
                    Sc=bc.damage.Sc
                S0=[-0.98,0.95*Sc] # S0- and S0+
                S1=[-0.99,1.05*Sc] # S1- and S1+
