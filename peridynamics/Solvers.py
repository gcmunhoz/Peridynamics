    ...
    for n in range(n_initial,len(t)-1):
        ...
        if b_Weval==True:
            ...
            if bc.bc_set.size>0:
                ...
                #vel_dof=bc.idb[bc.bc_set[bc.bc_set[:,2]==0,0]].astype(int) antigo
        ...
        if silent==False:
            ...
            pd._misc.progress_printer([(n+1)/(len(t)-1),t[n],(toc-tic)/(n+1)*(len(t)-(n+1))],fmt='Calculating... | Time: %v1 s | Percentage: |%b| %p % | ETA: %v2 s',bar_sep=40,hundred_percent_print=' Done.')
